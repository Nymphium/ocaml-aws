type input = Types.UpdateTableInput.t
type output = Aws.Json.t
type error = Errors_internal.t
let request_body_of_input input =
  Aws.Json.to_string (Types.UpdateTableInput.to_json input)
let service = "dynamodb"
let target = "DynamoDB_20120810.UpdateTable"
let signature_version = Aws.Request.V4
let headers = [("x-amz-target", target)]
let meth = `POST
let to_http service region req =
  let body = Some (Aws.Json.to_string (Types.UpdateTableInput.to_json req)) in
  let uri =
    Uri.of_string
